...
> `…/movie-cli-v5`, the installer should not blindly install **v5**. It should
...
> `movie-cli-v5`). Any AI implementing this MUST be able to do so from this
> document alone.
...
| Term | Meaning |
|------|---------|
| **Base name** | The repo name with the trailing `-v<N>` stripped. e.g. `movie-cli-v5` → `movie-cli` |
| **Current version** | The `<N>` in the URL the user invoked. e.g. `movie-cli-v5` → `5` |
...
|--------------|-------|------|---|
| `https://github.com/alimtvnetwork/movie-cli-v5` | `alimtvnetwork` | `movie-cli` | `5` |
...
```
[bootstrap] starting URL: https://github.com/alimtvnetwork/movie-cli-v5
...
```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/movie-cli-v5/main/bootstrap.ps1 | iex
```

### macOS / Linux

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/movie-cli-v5/main/bootstrap.sh | bash
```
...
- GIVEN URL `…/movie-cli-v5` AND v7 is the highest existing sibling
  WHEN bootstrap runs
  THEN it logs probes for v30..v8 as misses, v7 as HIT, and delegates to v7's `install.ps1`.

- GIVEN URL `…/movie-cli-v5` AND only v5 exists
  WHEN bootstrap runs
  THEN it logs misses for v30..v6, HIT for v5, and delegates to v5's `install.ps1`.

- GIVEN URL `…/movie-cli-v5` AND no sibling exists (network failure or all 404)
  WHEN bootstrap runs
